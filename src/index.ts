...
import { remoteSocketToWS, safeCloseWebSocket, sendMessage, vlessOverWSHandler } from './core/subServerService';
	...
	async fetch(request, env) : Promise<Response>{
		try {
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				if (new RegExp('/proxyip=', 'i').test(url.pathname)) AppParam.proxyIP = url.pathname.toLowerCase().split('/proxyip=')[1];
				else if (new RegExp('/proxyip.', 'i').test(url.pathname)) AppParam.proxyIP = `proxyip.${url.pathname.toLowerCase().split("/proxyip.")[1]}`;

				AppParam.socks5Address = url.searchParams.get('socks5') || AppParam.socks5Address;
				if (new RegExp('/socks5=', 'i').test(url.pathname)) AppParam.socks5Address = url.pathname.split('5=')[1];
